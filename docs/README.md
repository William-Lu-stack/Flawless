...
4. 保存后，SRE Run 和 AI 巡检会自动引用匹配到的 Skill。
...
3. 资源页面可选择不同集群。
4. 发布治理可以创建风险判定记录。
5. SRE Run 可以流式返回，并出现预演或可执行方案。

## 9. 运维 Skill 编写与添加指南

Skill 是把资深运维人员的经验注入平台的标准方式。当前实现兼容 Agent Skills 开放规范：每项 Skill 都是独立目录，以 `SKILL.md` 为跨智能体可读主体，以 `references/ops-policy.yaml` 保存 CISRE 的证据、动作和恢复门禁。SRE Run 按“元数据发现 -> 命中后加载正文”的渐进披露方式使用 Skill。
...
原有前端注入方式保持兼容：表单字段、保存接口、匹配测试、SRE Run 和 AI 巡检用法不变，只是底层从单一 JSON 改成标准目录包。
...
- SRE Run：用户描述问题后，平台会先匹配 Skill，再决定要补哪些证据和生成什么修复预演。
