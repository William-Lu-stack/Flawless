...
1. `registry.example.com/platform/cisre:5.5.0`
...
helm upgrade --install k8s-agent ./charts/flawless \
  ...
  --set-string image.tag=5.5.0 \
  --set-string config.NODE_EXEC_IMAGE=registry.example.com/platform/cisre-node-exec:1.36
